...
fun hashMap() {
     ...
     *
     * 2. create key.hashCode()
     *    generated hashCode will have the limited collisions - about 8
     *
     * 3. определяется позиция в массиве куда будет помещен элемент:
     *    indexFor(hash, tableLength) = hash & (tableLength - 1)
     *
     * 4. получаем цепочку элементов, привязанных к ячейке с данным индексом,
     *    сравниваются хеши и ключи, если совпадают, значение переписывается
     *
     * 5. если нет совпадений, добавляется новый элемент в ячейку
     ...
     *
     * 2. all table[0] cells are iterated for searching key=null element.
     *    if found, the value is rewritten
     *
